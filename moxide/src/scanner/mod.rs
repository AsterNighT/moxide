    ...
    ( $( $ty:ty ),* ) => {
        $(
            ...
            unsafe impl Scannable for $ty {
                ...
                fn format(&self, data:&[u8]) -> String {
                    format!("{}", unsafe { data.as_ptr().cast::<$ty>().read_unaligned() })
                }
...
impl<T: Scannable> Pattern<T> {
    ...
    pub fn run(&self, info: MEMORY_BASIC_INFORMATION, memory: Vec<u8>) -> Region {
        ...
        match self {
            ...
            Pattern::Unknown(size, _)
            | Pattern::Unchanged(size, _)
            | Pattern::Changed(size, _)
            | Pattern::Decreased(size, _)
            | Pattern::Increased(size, _) => Region {
                info,
            ...
            Pattern::DecreasedBy(value) | Pattern::IncreasedBy(value) => Region {
                info,
                locations: CandidateLocations::Dense {
                    range: base..base + info.RegionSize,
    ...
    pub fn rerun(&self, region: &Region, memory: Vec<u8>) -> Region {
        let size = match self {
            ...
            Pattern::Unknown(_, _) => return region.clone(),
            Pattern::Exact(value) => value.mem_view().len(),
            Pattern::InRange(low, high) => {
            ...
            | Pattern::Changed(size, _)
            | Pattern::Decreased(size, _)
            | Pattern::Increased(size, _) => *size,
            Pattern::DecreasedBy(value) | Pattern::IncreasedBy(value) => value.mem_view().len(),
        };
    ...
    unsafe fn acceptable(&self, old: &[u8], new: &[u8]) -> bool {
        match self {
            Pattern::Exact(value) => (value.scan_mode().eq)(value.mem_view(), new),
            Pattern::Unknown(_, _) => true,
            Pattern::InRange(low, high) => {
                let mode = low.scan_mode();
                let (low, high) = (low.mem_view(), high.mem_view());
                (mode.cmp)(low, new) != Ordering::Greater && (mode.cmp)(high, new) != Ordering::Less
            }
            Pattern::Unchanged(_, mode) => (mode.eq)(old, new),
            Pattern::Changed(_, mode) => !(mode.eq)(old, new),
            Pattern::Decreased(_, mode) => (mode.cmp)(old, new) == Ordering::Greater,
            Pattern::Increased(_, mode) => (mode.cmp)(old, new) == Ordering::Less,
            Pattern::DecreasedBy(value) => {
                let mode = value.scan_mode();
                let mut delta = old.to_vec();
                (mode.sub)(delta.as_mut(), new);
                (mode.eq)(value.mem_view(), delta.as_ref())
            }
            Pattern::IncreasedBy(value) => {
                let mode = value.scan_mode();
                let mut delta = old.to_vec();
...
impl FromStr for Ty {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        Ok(match value {
            ...
            "f32" | "F32" => Ty::F32,
            "f64" | "F64" => Ty::F64,
            _ => return Err(eyre!("Unknown type")),
        })
    }
}
...
impl Ty {
    ...
    pub fn default(&self) -> Box<dyn Scannable> {
        self.parse("0").unwrap()
    }
}

impl FromStr for Pattern<Box<dyn Scannable>> {
    type Err = eyre::Error;
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        ...
        Ok(match value.as_bytes()[0] {
            b'?' => Pattern::Unknown(size, mode),
            b'=' => Pattern::Unchanged(size, mode),
            b'!' => Pattern::Changed(size, mode),
            t @ b'd' | t @ b'i' => {
                let n = value[1..].trim();
                if n.is_empty() {
                    if t == b'd' {
                        Pattern::Decreased(size, mode)
                    } else {
                        Pattern::Increased(size, mode)
                    }
                } else {
                    let n = ty.parse(n)?;
                    if t == b'd' {
                        Pattern::DecreasedBy(n)
                    } else {
                        Pattern::IncreasedBy(n)
                    }
                }
            }
            _ => {
                ...
                if &low == &high {
                    Pattern::Exact(Box::new(low))
                } else {
                    Pattern::InRange(Box::new(low), Box::new(high))
                }
            }
        })
    }
...
impl Region {
    ...
    pub fn to_list(&self, ty:&Box<dyn Scannable>) -> Vec<(usize, String)> {
        let mut result = Vec::new();
        for addr in self.locations.iter() {
            let value = self.value_at(addr);
            let value = ty.format(value);
